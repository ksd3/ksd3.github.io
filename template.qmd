...
Edit the Typst source files (`cv/cv.typ` or `cv/resume.typ`), then compile directly into the output directory:

```bash
quarto typst compile cv/cv.typ docs/cv/cv.pdf
quarto typst compile cv/resume.typ docs/cv/resume.pdf
```

The `cv.qmd` landing page embeds the PDFs from `docs/cv/`. Find Typst templates at [typst.app/universe](https://typst.app/universe).
